...
$mail->addAddress('Anwarmuhammed45@gmail.com','Police');     // Add a recipient
...
$mail->Body    = '<body>The incident was uploaded to crimedetect website under the categery police in location '.$u.' with the following description '.$message.'. The image was checked for forgery and hope this can be a crucial evidence for the investigation</body>';
...
if(!$mail->send()) {
	...
	if($check == 1)
		{
			$sql = "DELETE FROM imgpolice WHERE filedestination='$filepath'";
			if (mysqli_query($conn, $sql)) {
						...
						if(!unlink($path)){
							...
							header("location:policeimgcheck.php?Email_Successfully_sended&delete_success");
						}
			}else {
