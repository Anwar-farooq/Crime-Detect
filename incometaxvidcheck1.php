...
set_time_limit(500);
	...
	$indexCount	= count($dirArray);
		...
		for($index=0; $index < $indexCount; $index++) {
			...
			$sql="select * from vidincometax where filename='$dirArray[$index]' and district='$u'";
			$r=mysqli_query($conn,$sql);
			$row= mysqli_fetch_array($r);
			if(in_array($extension,$allowed)){
			 if($row){
				echo '<tr class="active-row">';
				echo '<td><a href="viduploads/incometax/'.$u.'/' . $dirArray[$index] . '"><video id="myVideo" height="100px" width="100px" src="viduploads/incometax/'.$u.'/' . $dirArray[$index] . '" autoplay muted loop/>
				...
				echo '<td> <form action="vidcompare.php" method="POST">
							<input type="hidden" name="first" value="viduploads/incometax/'.$u.'/' . $dirArray[$index] . '">
							<input type="hidden" name="second" value="viduploads/incometax/'.$u.'/generated/'.$dirArray[$index].'">
						...
						<td> <form action="viddeleteincometax.php" method="post">
							<input type="hidden" name="varname" value='.$dirArray[$index].'>
							<input type="hidden" name="second" value="viduploads/incometax/'.$u.'/generated/'.$dirArray[$index].'">
							<input type="hidden" name="third" value="viduploads/copymove/">
							<input type="submit" value="delete" name="delete"></form> </td>
						<td><form enctype="multipart/form-data" method="POST" action="vidmailtoincometax.php"> 
					<input type="hidden" name="subject" value="Evidence for crime"/> 
					<input type="hidden" name="message" value='.$row["discription"].'>
					<input type="hidden" name="categery" value="incometax">
					<input type="hidden" name="filename" value="viduploads/incometax/'.$u.'/'.$dirArray[$index].'"/>
					<input type="hidden" name="second" value="viduploads/incometax/'.$u.'/generated/'.$dirArray[$index].'">
							<input type="hidden" name="third" value="viduploads/copymove/">
				...
				echo '<script type="text/javascript">alert("No video found in the categery incometax")</script>';
			}
			}
		}
