...
$mail->Body    = '<body>The incident was uploaded to crimedetect website under the categery '.$cat.' in location '.$u.' with the following description '.$message.'. The video was checked for forgery and hope this can be a crucial evidence for the investigation</body>';
...
if(!$mail->send()) {
	...
	if($check == 1)
		{
			$sql = "DELETE FROM vidnarcotics WHERE filedestination='$filepath'";
			if (mysqli_query($conn, $sql)) {
						...
						if(!unlink($path)){
							...
							header("location:narcoticsvidcheck.php?Email_Successfully_sended&delete_success");
						}
				}else {
