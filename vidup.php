<?php
		...
		if($type=="police"){
			if(in_array($fileActualExt,$allowed)){
				if($fileError===0){
					if($fileSize<50000000){
						$fileDestination='viduploads/police/'.$district.'/'.$filename;
			...
			if(in_array($fileActualExt,$allowed)){
				if($fileError===0){
					if($fileSize<50000000){
						$fileDestination='viduploads/forest/'.$district.'/'.$filename;
						move_uploaded_file($fileTmpName,$fileDestination);
						$conn = mysqli_connect("localhost","root","","crimedetect");
			...
			if(in_array($fileActualExt,$allowed)){
				if($fileError===0){
					if($fileSize<50000000){
						$fileDestination='viduploads/incometax/'.$district.'/'.$filename;
						move_uploaded_file($fileTmpName,$fileDestination);
						$conn = mysqli_connect("localhost","root","","crimedetect");
			...
			if(in_array($fileActualExt,$allowed)){
				if($fileError===0){
					if($fileSize<50000000){
						$fileDestination='viduploads/narcotics/'.$district.'/'.$filename;
						move_uploaded_file($fileTmpName,$fileDestination);
						$conn = mysqli_connect("localhost","root","","crimedetect");
