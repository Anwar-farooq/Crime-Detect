<?php
	...
	// open this directory 
	$myDirectory = opendir("imguploads/narcotics/$u");
	...
	$indexCount	= count($dirArray);
		...
		for($index=0; $index < $indexCount; $index++) {
			...
			$sql="select * from imgnarcotics where filename='$dirArray[$index]' and district='$u'";
			$r=mysqli_query($conn,$sql);
			$row= mysqli_fetch_array($r);
			if(in_array($extension,$allowed)){
				if($row){
					echo '<tr class="active">';
				echo '<td><a href="imguploads/narcotics/'.$u.'/' . $dirArray[$index] . '"><img height="100px" width="100px" src="imguploads/narcotics/'.$u.'/' . $dirArray[$index] . '" alt="Image" /></a></td>' ;
				echo '<td>'.$row["filename"].'</td>
						<td>'.$row["discription"].'</td>
						<td>'.$row["district"].'</td>
						<td> <form action="" method="post"><input type="submit" value="check" name='.$index.'></form> </td>
						<td> <form action="imgcompare.php" method="POST">
							<input type="hidden" name="first" value="imguploads/narcotics/'.$u.'/' . $dirArray[$index] . '">
							<input type="hidden" name="second" value="imguploads/narcotics/'.$u.'/generated/'.$dirArray[$index].'">
				...
				echo '<script>alert("No images found in the categery narcotics")</script>';
			}
			}	
		}
...
for($index=0; $index < $indexCount; $index++) {
	...
	{
		$n="imguploads/narcotics/$u";
		$m="imguploads/narcotics/$u/$dirArray[$index]";
